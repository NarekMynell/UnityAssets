param([string]$sdkManagerPath = "", [string]$javaHome="", [string]$sdkManagerArgs = "", [string]$sdkOutput = "")
...
Write-Host "Arguments are ""$sdkManagerPath"" ""$javaHome"" ""$sdkManagerArgs"" ""$sdkOutput"""
...
else
{
	...
	try
	{
		$args = "-ExecutionPolicy Bypass -File ""$PSCommandPath"" -ArgumentList Ignored ""$sdkManagerPath"" ""$javaHome"" ""$sdkManagerArgs"" ""$sdkOutput"""
